 ...
 * Supports: Millenia, Neu, and other HDFC cards
 */
export class HDFCParser extends BaseParser {
    ...
    parseStatement(text: string): StatementData {
        // Extract card name
        let cardName = 'HDFC'
        if (text.includes('Millenia') || text.includes('Millennia')) cardName = 'HDFC Millenia'
        else if (text.includes('Neu')) cardName = 'HDFC Neu'
        ...
        const paymentsMatch = text.match(/(?:Payments|Credits|Refunds)[:\s]*₹?\s*([0-9,]+\.?\d*)/i)
