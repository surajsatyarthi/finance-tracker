...
/**
 * Parser for RBL Bank credit card statements
 * Supports: Bajaj Finserv, Platinum Delight, and other RBL cards
 */
export class RBLParser extends BaseParser {
    ...
    parseStatement(text: string): StatementData {
        // Extract card name
        let cardName = 'RBL'
        if (text.includes('Bajaj Finserv')) cardName = 'RBL Bajaj Finserv'
        else if (text.includes('Platinum Delight')) cardName = 'RBL Platinum Delight'
        ...
        const purchasesMatch = text.match(/(?:Purchases|Charges)[:\s]*₹?\s*([0-9,]+\.?\d*)/i)
        const paymentsMatch = text.match(/(?:Payments|Credits)[:\s]*₹?\s*([0-9,]+\.?\d*)/i)
        const totalDueMatch = text.match(/Total (?:Amount )?[Dd]ue[:\s]*₹?\s*([0-9,]+\.?\d*)/i)
        const minDueMatch = text.match(/Minimum (?:Payment|Amount)[:\s]*₹?\s*([0-9,]+\.?\d*)/i)
