 ...
 * Supports: BPCL, Simply Save, and other SBI cards
 */
export class SBIParser extends BaseParser {
    ...
    parseStatement(text: string): StatementData {
        // Extract card name
        let cardName = 'SBI'
        if (text.includes('BPCL')) cardName = 'SBI BPCL'
        else if (text.includes('Simply Save') || text.includes('SimplySAVE')) cardName = 'SBI Simply Save'
